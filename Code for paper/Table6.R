...
MaxLags = 1 #Size of max lag (We choose lag 1)
PreTime = 1 #Set up 1-month ahead forecast
MaxPreTime = 6 #For consistency, do not change this value. (= 6)
Quantile_Level = c(1,0.875,0.75,0.625,0.5)
###################################Read Functions#####################################
...
W_Name = paste(FileSource,"W_RealDistance.csv",sep = "")
W_RealDist<-read.csv(W_Name,header = T)
W_RealDist<-as.matrix(W_RealDist)
W_RealDist = W_RealDist[,-1]

Test_MinDist = W_RealDist
diag(Test_MinDist) = 100000
MinDist = matrix(0,1,N)
for (i in 1:N){
MinDist[i] = min(Test_MinDist[i,])
}
MinDist = max(MinDist) #The minimum distance that make sure all LADs are connected

for (Sparsity in 1:length(Quantile_Level)){

Dist_Limit = as.integer(quantile(W_RealDist,Quantile_Level[Sparsity]))
#The output name for varying W
Name_VaringW = "VaryingW_"
Name_Sparsity = sprintf("sparsity_%s_",Quantile_Level[Sparsity])
Name_Dist_Limit = as.character(Dist_Limit)
Name_VaringW = paste(Name_VaringW,Name_Sparsity,Name_Dist_Limit,"kmLimits_",sep="")

if (Dist_Limit<MinDist){
  print("Error: The Distance of Selecting Quantile Level Must Not Below the Minimum Distance")
}

W_Spasity = matrix(0,N,N)
for (i in 1:N){
  for (j in 1:N){
    if (W_RealDist[i,j]<=Dist_Limit){
      W_Spasity[i,j] = 1/(W_RealDist[i,j])^2 #each entry equals the inverse of squared distance
    }
  }
}
diag(W_Spasity) = 0
...
for (nn in 1:tt)
{
  ...
  StrSparsity = as.character(Sparsity)
  TS = c("MaxSp_Num",length(Quantile_Level),"Sp_Num",StrSparsity,"TotalTime",StrT,"LoopTime",Str)
  print(TS)
...
for (p in 1:length(PreTime)){
  ...
  ModelResiduals_RMSE_Name = paste(OutputSource,Name_VaringW,"ModelResiduals_RMSE","_Lags",LagName,"_PreTime",PreTimeName,".xlsx",sep = "")
  ModelResiduals_ReMSPE_Name = paste(OutputSource,Name_VaringW,"ModelResiduals_ReMSPE","_Lags",LagName,"_PreTime",PreTimeName,".xlsx",sep = "")
  ModelResiduals_ReMSPE_Individual_Name = paste(OutputSource,Name_VaringW,"ModelResiduals_ReMSPE_Individual","_Lags",LagName,"_PreTime",PreTimeName,".xlsx",sep = "")
  ...
  LagName = as.character(Lags)
  PreTimeName = as.character(PreTime[p])
  ExcelFileName = paste(OutputSource,Name_VaringW,"DM_test_New","_Lags",LagName,"_PreTime",PreTimeName,".xlsx",sep = "")
