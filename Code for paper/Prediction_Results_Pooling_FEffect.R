...
MaxLags = 2 #Size of max lag (We choose maxlag=6 here, then it will run loops from lag 1 to lag 6)
PreTime = 1:6 #Set up 1-month to T-month ahead forecast
...
for (ct in 1:tt){ ARpcoef[[ct]] = as.matrix(matrix(0,N,N+Lags)) }
ARXpcoef = vector("list",tt)
for (ct in 1:tt){ ARXpcoef[[ct]] = as.matrix(matrix(0,N,N+(nvar+1)*Lags)) }
...
for (ct in 1:tt){ ARp_SE[[ct]] = as.matrix(matrix(0,N,N+Lags)) }
ARXp_SE = vector("list",tt)
for (ct in 1:tt){ ARXp_SE[[ct]] = as.matrix(matrix(0,N,N+(nvar+1)*Lags)) }
...
for (ct in 1:tt){ ARppvalue[[ct]] = as.matrix(matrix(0,N,N+Lags)) }
ARXppvalue = vector("list",tt)
for (ct in 1:tt){ ARXppvalue[[ct]] = as.matrix(matrix(0,N,N+(nvar+1)*Lags)) }
...
for (nn in 1:tt)
{
  ...
  ########Get Regression Results for ARpool (With FE) and AR-Xpool (With FE)
  #ARpool model (With FE)
  train_arp = getX_noX_HeterFE(Y, W, N, Lags, Wexsit = FALSE)
  if (Lags==1){
    equations = "Y ~ FE + X "
  }
  if (Lags>1){
    equations = "Y ~ FE + X.Xvec "
    for (fv in 2:Lags){
      
      xnum = as.character(fv)
      xnames = paste("+ X.V",xnum,sep="")
      equations = paste(equations,xnames,sep="")
      
    }
  }
  equations = formula(paste(equations,"- 1",sep=""))
  reg_arp = lm(equations, data = train_arp) #ARpool regression
  summary(reg_arp)
  Int_arp = summary(reg_arp)$coefficients[1:N] # Get ARpool regression intercept
  ARpcoef[[nn]][,1:N] = as.matrix( Int_arp ) # N fixed effects 'Int_arp' for N LADs, and store them
  pInt_arp = summary(reg_arp)$coefficients[(1+ncol(ARppvalue[[nn]])*3):(N+ncol(ARppvalue[[nn]])*3)] # Get p-value of ARpool regression intercept
  ARppvalue[[nn]][,1:N] = as.matrix( pInt_arp ) # Replicate N same p-value of 'pInt_arp' for N LADs, and store them
  Int_arp_SE = summary(reg_arp)$coefficients[(1+ncol(ARp_SE[[nn]])*1):(N+ncol(ARp_SE[[nn]])*1)] # Get SE of ARpool regression intercept
  ARp_SE[[nn]][,1:N] = as.matrix( Int_arp_SE ) # Replicate N same SE of 'Int_arp_SE' for N LADs, and store them
  for (c in (N+1):ncol(train_arp)){
  ...
  #AR-Xpool model (With FE)
  trainX_arp = getX_HeterFE(X, Y, W, N, Lags, Wexsit = FALSE)
  equations = "Y ~ FE + X.Xvec "
    for (fv in 2:((nvar+1)*Lags)){
  ...
  equations = formula(paste(equations,"- 1",sep=""))
  regX_arp = lm(equations, data = trainX_arp) #AR-Xpool regression
  summary(regX_arp)
  Intx_arp = summary(regX_arp)$coefficients[1:N] # Get AR-Xpool regression intercept
  ARXpcoef[[nn]][,1:N] = as.matrix( Intx_arp ) # Replicate N same intercept 'Intx_arp' for N LADs, and store them
  pxInt_arp = summary(regX_arp)$coefficients[(1+ncol(ARXppvalue[[nn]])*3):(N+ncol(ARXppvalue[[nn]])*3)] # Get p-value of AR-Xpool regression intercept
  ARXppvalue[[nn]][,1:N] = as.matrix( pxInt_arp ) # Replicate N same p-value of 'pxIntx_arp' for N LADs, and store them
  xInt_arp_SE = summary(regX_arp)$coefficients[(1+ncol(ARXp_SE[[nn]])*1):(N+ncol(ARXp_SE[[nn]])*1)] # Get SE of AR-Xpool regression intercept
  ARXp_SE[[nn]][,1:N] = as.matrix( xInt_arp_SE ) # Replicate N same SE of 'Intx_arp' for N LADs, and store them
  for (c in (N+1):ncol(trainX_arp)){
  ...
  x_pred_cbind = nx_pred_arp[(nrow(nx_pred_arp) - N*Lags+1):(nrow(nx_pred_arp)-N*(Lags-1)),] #Initialise a lag 1 In-Sample Predicted Data (with X)
    ...
    FixedEf = diag(N)
    n_pred_arp = cbind(FixedEf,pred_cbind) #Add Intercept Predicted Value (Identity matrix 'In')
    nx_pred_arp = cbind(FixedEf,x_pred_cbind) #Add Intercept Predicted Value (Identity matrix 'In')
...
for (p in 1:length(PreTime)){
  ...
  Col_str_RMSE = c("RMSE NAR","RMSE FE_ARpool","RMSE ARMG","RMSE AR","RMSE NAR-X","RMSE FE_AR-Xpool","RMSE AR-XMG","RMSE AR-X")
  colnames(Results_RMSE) = Col_str_RMSE
  
  Results_ReMSPE = matrix(0,15,8)
  Col_str_ReMSPE = c("ReMSPE NAR","ReMSPE FE_ARpool","ReMSPE ARMG","ReMSPE AR","ReMSPE NAR-X","ReMSPE FE_AR-Xpool","ReMSPE AR-XMG",
              "ReMSPE AR-X")
  colnames(Results_ReMSPE) = Col_str_ReMSPE
  
  Results_ReMSPE_Individual = matrix(0,15,8)
  Col_str_ReMSPE_Individual = c("ReMSPE NAR","ReMSPE FE_ARpool","ReMSPE ARMG","ReMSPE AR","ReMSPE NAR-X","ReMSPE FE_AR-Xpool","ReMSPE AR-XMG",
                                "ReMSPE AR-X")
  ...
  ModelResiduals_RMSE_Name = paste(OutPutSource,"FE_","ModelResiduals_RMSE","_Lags",LagName,"_PreTime",PreTimeName,".xlsx",sep = "")
  ModelResiduals_ReMSPE_Name = paste(OutPutSource,"FE_","ModelResiduals_ReMSPE","_Lags",LagName,"_PreTime",PreTimeName,".xlsx",sep = "")
  ModelResiduals_ReMSPE_Individual_Name = paste(OutPutSource,"FE_","ModelResiduals_ReMSPE_Individual","_Lags",LagName,"_PreTime",PreTimeName,".xlsx",sep = "")
  ...
  LagName = as.character(Lags)
  PreTimeName = as.character(PreTime[p])
  ExcelFileName = paste(OutPutSource,"FE_","DM_test_New","_Lags",LagName,"_PreTime",PreTimeName,".xlsx",sep = "")
  write.xlsx(EmptyFile,file = ExcelFileName,col.names = FALSE,row.names = FALSE)
  
  ResidualName = c("NAR","NARX","FE_ARp","FE_ARXp","AR","ARX","ARMG","ARXMG")
  for (res1 in 1:7){
...
for (r in 1:15){
  ...
  RegionExcelName_RMSE = paste(OutPutSource,"FE_","RMSE_",RegionName,"_MaxLags",as.character(MaxLags),"_MaxPreTime",as.character(length(PreTime)),".xlsx",sep = "")
  write.xlsx(Regional_RMSE_Residuals,file = RegionExcelName_RMSE)
  
  RegionExcelName_ReMSPE = paste(OutPutSource,"FE_","ReMSPE_",RegionName,"_MaxLags",as.character(MaxLags),"_MaxPreTime",as.character(length(PreTime)),".xlsx",sep = "")
  write.xlsx(Regional_ReMSPE_Residuals,file = RegionExcelName_ReMSPE)
  
  RegionExcelName_ReMSPE_Ind = paste(OutPutSource,"FE_","ReMSPE_Ind_",RegionName,"_MaxLags",as.character(MaxLags),"_MaxPreTime",as.character(length(PreTime)),".xlsx",sep = "")
